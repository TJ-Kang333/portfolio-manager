...
## Phase 3c — 미래에셋 주식 시세 자동조회 (2026-09-05)

사용자가 실제 미래에셋 "보유종목" 내보내기(.xls, Excel COM으로 직접 열어 구조 확인)를
줌 — 기대보다 좋은 데이터: 종목번호(예: A069500), 보유량, 평균단가, 현재가가 전부 있음.

### 클라이언트 (`index.html`)
- CSV 가져오기에 **새 형식 자동 인식**: 헤더에 "종목번호"+"보유량"이 있으면 이 경로로 처리.
  국내(주식/ETF/ETN)는 종목번호 앞의 "A" 제거해 순수 코드(069500)로, 해외(소수해외 등)는
  티커 그대로(TSLA) 저장. 기존 "종합잔고"/"단순 헤더" 형식은 그대로 유지, 셋 다 공존.
- `applyImport()`가 이제 qty/avgPrice/currentPrice 를 `S.months[...][ticker]`에 같이 저장
  (기존에 이미지 인식 경로만 쓰던 필드 — CSV 경로도 채우도록 확장).
- 대시보드에 **"⟳ 시세 갱신"** 버튼: 최신 월에서 **수량이 있는** 종목만 골라 서버에서 현재가를
  받아와 `balance = qty × 현재가`로 즉시 재계산. 수량 없이 CSV로 금액만 들어온 종목은 그대로 둠
  (자동 계산할 근거가 없어서).
- 예수금/현금은 원래 있던 "💰 예수금 추가 입력" 필드로 계속 처리 — 이 CSV엔 안 잡히는 게 정상.
- 검증: 사용자 실제 파일 구조 그대로(콤마 포함 숫자의 CSV 따옴표 처리 포함) parseAndRender →
  applyImport → refreshStockPrices 전체 경로를 브라우저에서 통과.

### 서버 (`apps-script/stocks.gs`, 새 파일)
- `fetchStockPricesKRW(tickers)`: 6자리 코드는 국내로 판단해 **네이버 금융 폴링 API**로 한 번에
  조회, 그 외(TSLA 등 영문 티커)는 **Yahoo Finance 차트 API**로 개별 조회 후 환율 곱해 원화 통일.
  둘 다 키 불필요.
- `Code.gs` doGet: `?action=stock_prices&tickers=069500,160580,TSLA`.
- `snapshotStockPrices()` + `setupStockSnapshotTrigger()`: 매일 16시(장마감 후) **state는 읽기만
  하고**(안 씀) 최신 월 보유종목을 재평가해 `자산스냅샷`에 한 줄만 남김 — state(공유 데이터)를
  서버가 직접 고치게 하면 클라이언트 저장 시 되돌아갈 위험이 있어서(Phase 1 충돌병합이 로컬 우선),
  일부러 "기록만" 하고 실제 반영은 항상 클라이언트의 "⟳ 시세 갱신"이 하게 설계.
- `_testStockPrices()`: 사용자의 실제 종목코드로 편집기에서 바로 점검용.

### 확인 필요 (다음 라운드)
네이버/Yahoo 엔드포인트가 실제로 도는지는 서버에서 돌려봐야 앎 — OKX 때처럼 한 번에 안 맞으면
조정 예정. `_testStockPrices` 실행 결과 확인 필요.

### 사용자가 할 것
1. `stocks.gs` 새 파일 추가, `Code.gs` 교체 → 새 버전 배포.
2. 편집기에서 `_testStockPrices` 실행 → 로그에 6개 종목 다 가격이 찍히는지 확인, 안 되면 결과 공유.
3. `git push` (index.html 바뀜).
4. 앱에서 CSV(종목번호 포함 형식)로 잔고 넣고 → 대시보드 "⟳ 시세 갱신" 눌러 확인.

## Phase 3b — 은행 잔고/입출금 알림 + 계좌이체 자동 병합 (2026-09-04)

### 서버 (`apps-script/email_parser.gs`, `Code.gs`)
- **우리은행/하나은행 파서** 추가 (`srcKey: 'woori' / 'hana_bank'`). 두 은행이 알림 순서가
  달라("카카오페이 | 우리 …" vs "1,283,738 원 입금 | 강태준") 순서 무관하게 파싱하는
  `parseBankAlert()` 공용 함수로 처리. 잔액이 알림에 같이 오면(`잔액 N원`) 거래 기록과
  별도로 `자산스냅샷`에도 자동 기록.
- **광고성 알림 자동 제외**: "입금/출금" + "원" 패턴이 없으면 `parse()`가 `null` 반환 —
  `source` 힌트로 파서를 강제해도 마찬가지라 광고가 거래로 잘못 기록될 수 없음.
- `가계부거래` 시트에 9번째 컬럼 **"시각"**(ISO timestamp) 추가 — 계좌이체 짝 맞추기에 사용.
  기존 시트도 데이터는 정상 기록됨(헤더 행 라벨만 안 바뀔 수 있음, 무해).
- `normalizeSourceHint`: `woori`/`hana_bank`(은행)와 `hana_card`(카드, 추후) 구분 —
  MacroDroid URL엔 `&source=hana_bank` 로 명시.

### 클라이언트 (`index.html`) — 계좌이체 자동 병합
- 서로 다른 은행에서 **금액이 같고 방향이 반대(출금↔입금)이며 시각이 ±15분 이내**인 알림 쌍을
  발견하면, 지출+수입 두 건이 아니라 **`type:'transfer'`(이체) 한 건**으로 합쳐서 기록.
  기존 앱의 이체 메커니즘(`toAcctId`, 지출/수입 합계에서 자동 제외)을 그대로 사용 — 새 개념
  아님.
- 두 계좌 중 하나라도 아직 매핑 안 됐으면 병합하지 않고 각각 검토 목록으로 (안전한 쪽으로 폴백).
- 검증: 정상 병합(잔액 반영 확인) / 계좌 미상 폴백 / 시간差 큰 경우 병합 안 함 3케이스 브라우저 확인.
- 한계: 같은 배치(한 번의 "가져오기")에 두 알림이 같이 들어와야 병합됨. 한쪽이 크게 늦게 오면
  개별 지출/수입으로 기록됨 — 나중에 수동으로 "이체"로 고칠 수 있음(기존 UI).

### 사용자가 할 것
1. `Code.gs`, `email_parser.gs` 최신으로 교체 → 새 버전 배포.
2. MacroDroid 매크로 2개 추가 (경기지역화폐와 동일한 "알림 수신" 방식):
   - 우리WON뱅킹 앱 알림 → `?action=ingest&source=woori`
   - 하나원큐 앱 알림 → `?action=ingest&source=hana_bank`
3. `git push` (index.html 바뀜).
