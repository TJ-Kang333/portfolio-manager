...
## 2026-08-30 업데이트 — 실제 배포된 Apps Script 확인

사용자가 배포해 둔 실제 코드를 확보. 사본: `apps-script/Code.gs`. 위 감사 대비 정정:

### A2 (5만 자 한도) — 해결됨
- 실제 배포본은 JSON 을 45,000자 청크로 쪼개 `state` 시트 A2, A3… 에 나눠 저장 (`saveState`, `CHUNK_SIZE`).
- 단, `sh.clearContents()` 후 청크를 한 칸씩 `setValue` → 원자적이지 않음(중간 실패 시 깨짐). 큰 문제는 아님.

### A1 (이메일 파서) — 읽기쪽만 존재, 수집쪽이 없음  ← 목표 1 미작동의 확정 원인
- 실제 배포본에는 `loadEmailTxns()` / `markEmailDone()` 가 있음. `가계부거래` 시트에서 미반영 행을 읽어 앱에 넘겨줌.
  - `가계부거래` 컬럼: A=날짜 B=유형 C=가맹점 D=금액 E=출처 F=원문 (G=미사용) H=상태('done')
- **그러나 Gmail 을 스캔해 `가계부거래` 시트에 행을 쓰는 코드가 전혀 없음.** `GmailApp` 호출 0건, 시간 트리거 0건.
- 결과: `가계부거래` 시트는 항상 비어 있고 "미반영 거래 가져오기" 는 늘 `rows: []` → "미반영 거래가 없습니다." 무한 반복.
- **Phase 2 = 이 수집 함수(`parseCardEmails`)를 새로 작성**해서 트리거로 돌리는 것. 읽기쪽 계약(위 8컬럼)은 이미 정해져 있으니 거기 맞춰 쓰면 됨.

### A3 (동기화 충돌) — 그대로 유효
- `saveState` 는 리비전 확인 없이 전체 덮어쓰기. `doGet load` 는 전체 state 반환. 타임스탬프/리비전 없음.

### D1 (인증 토큰 없음) — 그대로 유효
- 웹앱 액세스 = "모든 사용자", 토큰 파라미터 없음. URL 아는 사람은 전체 접근 가능.
